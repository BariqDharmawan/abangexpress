            ...
            <div class="col-lg-3 mb-4">
                <div class="card h-full">
                    <div class="card-header">
                        <img src="{{ $person->avatar }}" height="90px"
                        class="rounded-circle mx-auto d-block" alt="{{ $person->name }}">
...
@section('components')
    <x-admin.modal id="add-new-person" 
        ...
        @include('admin.team.form', ['action' => '', 'data' => ''])
    </x-admin.modal>
    @foreach ($teams as $person)
        <x-admin.modal id="edit-person-{{ $loop->iteration }}" 
            heading="Edit info {{ $person->name }}">
            @include('admin.team.form', ['action' => '', 'data' => $person])
        </x-admin.modal>
