        ...
        <x-admin.table class="datatable-disable-pagination-ordering" :thead="[
            ...
            <tr>
                ...
                <td>
                    ...
                    @else
                    <img alt="" height="100px" src="{{ asset($identity->cover_vision_mission) }}">
                    @endif
                </td>
            </tr>
        </x-admin.table>
...
<x-admin.modal id="edit-idendity" heading="Ubah tentang kita">
    <form method="POST" enctype="multipart/form-data" action="{{ route('admin.about-us.update') }}">
        @csrf @method('PUT')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="edit-identity-vision" name="our_vision" required
        ...
        <div class="form-group">
            ...
            <textarea name="our_mission" id="edit-mission" class="form-control summernote" style="resize: none;"
                rows="3" required>{{ old('our_mission') ?? $identity->our_mission }}</textarea>
            @error('our_mission')
            <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="edit-heading" name="section_name" required
        ...
        <div class="form-group">
            ...
            <textarea name="second_desc" id="edit-second-desc" class="form-control summernote" style="resize: none;"
                rows="3" required>{!! $aboutUs->second_desc !!}</textarea>
            @error('second_desc')
            <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>

        <x-admin.input label="Ubah video youtube" name="our_video" placeholder="Taruh video youtube disini"
            value="{{ old('our_video') ?? $identity->our_video }}" required></x-admin.input>

        <div class="form-group">
            <img src="{{ asset($identity->cover_vision_mission) }}" height="100px" alt="" class="mb-3">
            <div class="custom-file">
                <input type="file" class="custom-file-input" name="cover_vision_mission" id="cover">
